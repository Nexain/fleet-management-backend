...
func (r *LocationRepository) SaveLocation(ctx context.Context, location *models.Location) error {
	fmt.Println("[DB][SaveLocation] Saving location to DB")
	query := `
...
func (r *LocationRepository) GetLastLocation(ctx context.Context, vehicleID string) (*models.Location, error) {
	query := `
...
func (r *LocationRepository) GetLocationHistory(ctx context.Context, vehicleID string, start, end int64) ([]models.Location, error) {
	query := `
        SELECT vehicle_id, latitude, longitude, timestamp
        FROM vehicle_locations
